...
> Caution! This is not an official plugin. It's a simple snippet, but it affects every page in WordPress. Standalone shouldn't be a huge problem, but when used with the Cache plugin, it's a different story. A malfunctioning page Cache can cause serious security problems. It is only recommended for professionals who can directly view the generated page Cache file. Developer has no take responsibility for management. This plugin user has responsibility for all problems.
